...
describe(chalk.blue('Parse and evaluate decision tables'), function () {
  ...
  it('Evaluation decision literal expression', function (done) {
    decisionTable.parseDmnXml(readFile("./test/data/test-literal-expression.dmn")).then(decisions => {
      expect(decisions['decisionTestVariable']).not.to.be.undefined;
      const context = {
        input: {
          category: "E",
      ...
      let data = decisionTable.evaluateDecision('decisionTestVariable', decisions, context);
      expect(data.testvariable).to.equal("Testing");

      data = decisionTable.evaluateDecision('depedentVar', decisions, context);
      expect(data.depedentVar).to.be.true;

      data = decisionTable.evaluateDecision('decisionTableBasedOnVariables', decisions, context);
      expect(data.output).to.equal('True Testing');
  ...
  it('Evaluation decision table with required decision with merged results', function (done) {
    decisionTable.parseDmnXml(readFile("./test/data/test.dmn")).then(decisions => {
      expect(decisions['decisionPrimary']).not.to.be.undefined;
      const context = {
        input: {
      ...
      let output = decisionTable.evaluateAllDecisions(decisions, context);
      let data = output['decisionPrimary'];
      expect(data.output.score).to.equal(50);
